...
import { Instagram, Music, Globe, Flag, Users, Sparkles, Copy, QrCode as QrCodeIcon, X, Download, Check, AlertCircle } from "lucide-react"
...
const QRCodeModal = ({ walletAddress, isOpen, onClose }: { walletAddress: string; isOpen: boolean; onClose: () => void }) => {
      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl shadow-2xl max-w-md w-full mx-4 overflow-hidden border border-yellow-400/30"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="px-6 py-4 flex items-center justify-between bg-gradient-to-r from-slate-700/50 to-slate-800/50 border-b border-slate-600/50">
              ...
              <Button
                ...
                size="sm"
                onClick={onClose}
                className="w-8 h-8 p-0 rounded-lg bg-slate-600/50 hover:bg-slate-500/50 text-slate-300 hover:text-white transition-all duration-200"
              >
                <X className="w-4 h-4" />
              </Button>
            </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex justify-center">
                <motion.div
                  ...
                  className="relative"
                >
                  <div className="w-56 h-56 flex items-center justify-center bg-white rounded-2xl p-4 shadow-xl border-4 border-yellow-400/20">
                    {error ? (
              ...
              <motion.div
                ...
                className="space-y-3"
              >
                <p className="text-sm font-medium text-slate-300 text-center">Wallet Address</p>
                <div className="relative group">
                  <div className="bg-slate-700/50 rounded-xl p-4 border border-slate-600/50 backdrop-blur-sm">
                    <p className="text-xs font-mono text-slate-200 break-all leading-relaxed text-center">
                      {walletAddress}
                    </p>
                  </div>
                  <div className="absolute inset-0 bg-gradient-to-r from-yellow-400/5 to-transparent rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                </div>
              </motion.div>
              ...
              >
                <Button
                  ...
                  className="flex-1 bg-yellow-400/10 hover:bg-yellow-400/20 text-yellow-400 border border-yellow-400/30 hover:border-yellow-400/50 transition-all duration-200"
                  variant="outline"
                >
                  {copied ? (
                    <>
                ...
                {qrDataUrl && (
                  <Button
                    onClick={downloadQR}
                    className="flex-1 bg-slate-600/50 hover:bg-slate-500/50 text-slate-200 border border-slate-500/50 hover:border-slate-400/50 transition-all duration-200"
                    variant="outline"
                  >
              ...
              >
                Scan this QR code with your wallet app or copy the address above to receive payments
              </motion.p>
            </div>
          </motion.div>
        </motion.div>
